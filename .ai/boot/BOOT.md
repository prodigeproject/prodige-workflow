...
6. **Load behavioral skills (MANDATORY):**
   - `.ai/skills/karpathy-behavioral.md`
   - **🆕 `.ai/skills/test-driven-development.md`** (TDD enforcement)
   - **🆕 `.ai/skills/verification-before-completion.md`** (Evidence requirement)
   - **🆕 `.ai/skills/systematic-debugging.md`** (For /fix workflow)
7. Load relevant context:
   - PROJECT.md
   - PRD.md
   - ARCHITECTURE.md
   - IMPLEMENTATION.md
   - CONTEXT.md (NEW) - Domain glossary if exists
   - docs/adr/*.md (NEW) - Architecture Decision Records if exist
9. Load state:
   - CURRENT.md
   - SPRINT.md
   - BACKLOG.md
...
14. **Apply Karpathy behavioral principles at all stages.**
...
- **🆕 Create ADR for hard-to-reverse architectural decisions** - Use docs/adr/ format.
...
### Quality Enforcement Rules (Superpowers)
...
**Note:** Orchestrator automatically loads skills based on command. Agents don't choose - skills are mandatory protocols.
...
**Status:** All phases now active in skill-selection matrix ✅
